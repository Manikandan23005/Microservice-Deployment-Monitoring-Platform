...
class AIAgentPipeline:
    ...
    def run_pipeline(
        ...
        if pod_status in ["CrashLoopBackOff", "Error"] or restarts > 0:
            ...
            severity = "Critical"
            root_cause_text = f"Pod '{pod_data['name']}' experienced container restarts ({restarts} restarts, status: {pod_status}). Previous container terminated with exit code {evidence['container_status']['last_exit_code']} due to database connection refusal or startup exception."
        elif pod_status in ["ImagePullBackOff", "ErrImagePull"]:
            incident_type = "ImagePullBackOff"
            severity = "High"
            root_cause_text = f"Kubelet failed to pull container image '{pod_data['image']}'. Registry image tag does not exist or imagePullSecrets are missing."
